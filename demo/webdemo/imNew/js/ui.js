var appUI = {
  ...
  makeChatContent: function(message, user) {
    ...
    var showNick = message.scene === 'team' && from !== userUID;
    if (
      //通知类消息
      message.attach &&
      message.attach.type &&
      ...
      if (message.type === 'g2') {
        let g2Text = '';
        if (message.attach.status === 1) {
          let duration = message.attach.durations[0].duration
          let m = Math.floor(duration / 60)
          let s = Math.floor(duration % 60)
          m = m < 10 ? ('0' + m) : m
          s = s < 10 ? ('0' + s) : s
          g2Text = '通话时长 ' + m + ':' + s;
        } else if (message.attach.status === 2) {
          g2Text = '已取消'
        } else if (message.attach.status === 3) {
          g2Text = '已拒绝'
        } else if (message.attach.status === 4) {
          g2Text = '未接听'
        } else if (message.attach.status === 5) {
          g2Text = '占线呼叫'
        }
        msgHtml = [
          '<div data-time="' +
            ...
            '" data-idServer="' +
            message.idServer +
            '" class="item item-' + (from === userUID ? 'me' : 'you') + '">',
  ...
  buildmeetingMemberUI: function(data) {
    ...
    return (
      ...
      data.account +
      '">' +
      (data.account === userUID ? '' : '<img class="audioMute-icon" src="images/audio-unmute.png" />') +
      data.nick +
      '</p>' +
