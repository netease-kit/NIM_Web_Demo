...
(function() {
  ...
  puer = module.exports = function(options) {
    var addon, app, dev, genQr, ifaces, key, proxy, ref, restRoute, server;
    if (options == null) {
      options = {};
    }
    ...
    if (!options.proxy) {
      ref = helper.requireFolder(path.join(__dirname, "./addons"));
      for (key in ref) {
    ...
    if (options.addon) {
      ...
      restRoute = function() {
        var callback, e, method, pt, ref1, tmp, type;
        try {
          ...
          ref1 = addon.routes;
          for (pt in ref1) {
            if (!hasProp.call(ref1, pt)) continue;
            callback = ref1[pt];
